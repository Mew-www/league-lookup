...
export class PreGameTeammateComponent implements OnInit {
  ...
  ngOnInit() {
    this.metadata.champions$
      .first()
      .subscribe(champions_container => {
        this.buffered_requests.buffer(() => {
          return this.player_api.getListOfRankedGamesJson(this.summoner.region, this.summoner.account_id, GameType.SOLO_AND_FLEXQUEUE_5V5, champions_container)
        })
          .subscribe(api_res => {
